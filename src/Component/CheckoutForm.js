...
const CheckoutForm = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!zip) {
            newErrors.shippingInfo.zip = "Zip code is required";
            errorsFound = true;
        } else if (!/^\d{6}$/.test(zip)) {
            newErrors.shippingInfo.zip = "Zip code must be 6 digits";
            errorsFound = true;
        }
        ...
        if (!billingZip) {
            ...
            errorsFound = true;
        } else if (!/^\d{6}$/.test(billingZip)) {
            newErrors.billingInfo.zip = "Zip code must be 6 digits";
            errorsFound = true;
        }
        ...
        alert(`Form submitted susscessfully with values: ${JSON.stringify(order)}`);
        // Reset
        setShippingInfo({
            firstName: "",
            lastName: "",
